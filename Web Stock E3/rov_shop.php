...
// ดึงข้อมูลสินค้า ROV เท่านั้น
$query = "SELECT * FROM Accounts WHERE status = 'available' AND game_id = '#ROV' ORDER BY account_id ASC";
        ...
        if ($user_point < $total_price) {
            ...
            try {
                ...
                foreach ($_SESSION['cart'] as $item) {
                    ...
                    $game_id = 'ROV'; // กำหนดให้เป็น ROV
                    $insert_stmt = $conn->prepare("INSERT INTO purchase_history (user_id, account_id, game_id, price, purchase_date) VALUES (?, ?, ?, ?, ?)");
                    if (!$insert_stmt) {
                        throw new Exception("Prepare insert statement failed: " . $conn->error);
                    }
                    
                    $insert_stmt->bind_param("sssds", $user_id, $account_id, $game_id, $price, $purchase_date);
...
<head>
    ...
    <style>
        body {
            ...
            margin: 0;
            padding: 0;
            background: linear-gradient(135deg, #1E2D70, #0D103D);
            color: white;
            line-height: 1.6;
        }
        ...
        .navbar {
            ...
            align-items: center;
            padding: 15px 20px;
            background: #0D103D; /* ROV dark blue */
            box-shadow: 0px 4px 10px rgba(0, 0, 0, 0.2);
            border-bottom: 2px solid #F8BA00; /* ROV gold */
        }
        ...
        .user-info {
            ...
            padding: 8px 12px;
            border-radius: 5px;
            border: 1px solid #F8BA00; /* ROV gold border */
        }
        ...
        .header h1 {
            ...
            margin-bottom: 10px;
            color: #F8BA00; /* ROV Gold */
            text-shadow: 0 0 10px rgba(248, 186, 0, 0.5);
        }
        ...
        .modal h2 {
            color: #F8BA00;
            margin-top: 0;
            border-bottom: 2px solid rgba(248, 186, 0, 0.3);
            padding-bottom: 15px;
        }
...
<body>
    <!-- Navbar -->
    <div class="navbar">
        <div class="brand">🎮 ROV SHOP</div>
        <div class="nav-right">
            <?php if ($logged_in): ?>
                <div class="user-info">
                    👤 <?php echo $_SESSION['username'] ?? 'User'; ?> 
                    <span style="color: #F8BA00; font-weight: bold;"><?php echo number_format($user_point, 2); ?> Point</span>
                </div>
    ...
    <!-- Header -->
    <div class="header">
        <h1>ARENA OF VALOR ACCOUNTS SHOP</h1>
        <p>เลือกซื้อไอดีเกม ROV คุณภาพดี การันตีทุกบัญชี</p>
    </div>
        ...
        <div class="products">
            ...
            <?php else: ?>
                <div style="grid-column: 1 / -1; text-align: center; padding: 40px;">
                    <h2 style="color: #F8BA00;">ไม่มีสินค้า ROV ในขณะนี้</h2>

...
// ดึงข้อมูลสินค้า VLR เท่านั้น
$query = "SELECT * FROM Accounts WHERE status = 'available' AND game_id = '#VLR' ORDER BY account_id ASC";
    ...
    if (empty($_SESSION['cart'])) {
        ...
        if ($user_point < $total_price) {
            ...
            try {
                ...
                $_SESSION['points'] = $new_point; // ใช้ points แทน user_point
                ...
                // ดึงข้อมูลสินค้าใหม่ - ยังคงดึงเฉพาะสินค้าที่มีสถานะ available
                $result = mysqli_query($conn, $query);
...
<body>
    <!-- Navbar -->
    <div class="navbar">
        <div class="brand">🎮 VLR SHOP</div>
        <div class="nav-right">
            <?php if ($logged_in): ?>
                <div class="user-info">
                    👤 <?php echo $_SESSION['username'] ?? 'User'; ?> 
                    <span style="color: #8affb6; font-weight: bold;"><?php echo number_format($user_point, 2); ?> Point</span>
                </div>
    ...
    <div class="header">
        ...
        <p>เลือกซื้อไอดีเกม VALORANT คุณภาพดี การันตีทุกบัญชี</p>
    </div>
        ...
        <div class="products">
            ...
            <?php else: ?>
                <div style="grid-column: 1 / -1; text-align: center; padding: 40px;">
                    <h2>ไม่มีสินค้า VLR ในขณะนี้</h2>
